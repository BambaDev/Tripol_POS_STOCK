...
namespace Pos.Forms.Customer
{
    public partial class Payment : DevExpress.XtraEditors.XtraForm
    {
        ...
        private void btnPay_Click(object sender, EventArgs e)
        {
            if (dxValidationProvider.Validate())
            {
                ...
                if (this.customers != null)
                    ProcessSalePayment(this.customerId, amountPaid);
        ...
        public void ProcessSalePayment(int customerId, decimal paymentAmount)
        {
            if (paymentAmount <= 0)
            {
            ...
            using (var context = new AppDbContext())
            {
                using (var transaction = context.Database.BeginTransaction())
                {
                    try
                    {
                        // Fetch the customer along with their outstanding SalePayments
                        var customer = context.Customers
                            .Include(c => c.SalePayments.Where(sp => sp.Due > 0)) // Only load unpaid sales
                        ...
                        decimal totalDueAfterPayment = 0;

                        // Process each sale payment, ordered by the due date (older sales first)
                        foreach (var salePayment in customer.SalePayments.OrderBy(sp => sp.DueDate))
                        {
                            if (remainingPayment <= 0) break; // Stop if there's no more payment to allocate

                            // Determine how much to apply to this sale
                            decimal amountToPay = Math.Min(salePayment.Due.Value, remainingPayment);

                            // Apply the payment
                            salePayment.Due -= amountToPay; // Reduce due amount
                            salePayment.Paid = (salePayment.Paid ?? 0) + amountToPay; // Increase paid amount
                            salePayment.Remaining = salePayment.Due; // Update remaining due amount

                            // Mark the sale as fully paid if due is zero
                            if (salePayment.Due == 0)
                            {
                                salePayment.IsPaid = true;
                            }

                            // Decrease remaining payment
                            remainingPayment -= amountToPay;

                            // Update total due after this payment
                            totalDueAfterPayment += salePayment.Due.Value;
                        }

                        // Update the customer's current due
                        customer.CurrentDue = totalDueAfterPayment;
                    ...
                    catch (Exception ex)
                    {
                        ...
                        throw new InvalidOperationException("An error occurred during the sale payment process.", ex);
                    }
                }
            }
        }
