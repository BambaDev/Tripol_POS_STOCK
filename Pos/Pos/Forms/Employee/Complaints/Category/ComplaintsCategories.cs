...
{
    public partial class ComplaintsCategories : DevExpress.XtraBars.Ribbon.RibbonForm
    {
        ...
        private void MenuItemAdd_Click(object sender, EventArgs e)
        {
            ...
            {
                AddEditComplaintsCategory category = new AddEditComplaintsCategory();
        ...
        public void loadCategories()
        {
            using (var context = new AppDbContext())
            {
                ...
                int totalItems = context.ComplaintCategories
        ...
        private void BindDataToGrid(int pageNumber)
        {
            using (var context = new AppDbContext())
            {
                ...
                var currentPageData = context.ComplaintCategories
                    .Where(p => p.Name.Contains(searchTerm))
        ...
        {
            if (Function.Permission.HasPermission("Add Complaints Category"))
            {
                string lang = Properties.Settings.Default.Lang;
                ...
                {
                    AddEditComplaintsCategory category = new AddEditComplaintsCategory(category_id);
        ...
        public void categoryDelete()
        {
            ...
            if (Function.Permission.HasPermission("Delete Complaints Category"))
            {
                using (var context = new AppDbContext())
                {
                    btnCategoryEdit.Enabled = true;
                    ...
                    Models.ComplaintCategory category = context.ComplaintCategories.Find(this.category_id);
        ...
        private void perPage_SelectedIndexChanged(object sender, EventArgs e)
        {
            if (int.TryParse(perPage.SelectedItem.ToString(), out int newItemsPerPage))
            {
                using (var context = new AppDbContext())
                {
                    ...
                    int totalItems = context.ComplaintCategories.Count();
                    totalPages = (int)Math.Ceiling((double)totalItems / itemsPerPage);
                    BindDataToGrid(currentPage);
                }
            }
        ...
        private void btnAddItem_Click(object sender, EventArgs e)
        {
            if (Function.Permission.HasPermission("Add Complaints Category"))
            {
                AddEditComplaintsCategory addEditComplaintsCategory = new AddEditComplaintsCategory();
                addEditComplaintsCategory.ShowDialog();
                loadCategories();
            }
           
        }

        private void gridViewCategories_RowClick(object sender, RowClickEventArgs e)
        {
            using (AppDbContext AppDb = new AppDbContext())
            {
                idComplaintCategories = int.Parse(gridViewCategories.GetRowCellValue(gridViewCategories.FocusedRowHandle, "Id").ToString());
            }
        }
    }
}