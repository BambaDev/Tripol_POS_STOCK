...
{
    public partial class Complaints : DevExpress.XtraBars.Ribbon.RibbonForm
    {
        ...
        public void loadComplaints()
        {
            using (var context = new AppDbContext())
            {
                ...
                int totalItems = context.Complaints
        ...
        private void BindDataToGrid(int pageNumber)
        {
            using (var context = new AppDbContext())
            {
                ...
                var currentPageData = context.Complaints
                    .Where(p => p.Title.Contains(searchTerm) ||
                        p.Description.Contains(searchTerm))
        ...
        {
            if (Function.Permission.HasPermission("Edit Complaints"))
            {
                string lang = Properties.Settings.Default.Lang;
                ...
                {
                    AddEditComplaints expense = new AddEditComplaints(expense_id);
        ...
        public void ComplaintDelete()
        {
            if (Function.Permission.HasPermission("Delete Complaints"))
            {
                if (gridViewExpenses.RowCount > 0 && gridViewExpenses.FocusedRowHandle >= 0)
                {
                    string lang = Properties.Settings.Default.Lang;
                    ...
                    if (lang == "en")
                    {
                        ...
                        areYouSure = "Are you sure want to delete Complaint ?";
                    }
                    else if (lang == "fr")
                    {
                        confirmation = "Confirmation";
                    ...
                    if (Function.Permission.HasPermission("Delete Complaints"))
                    {
                        using (var context = new AppDbContext())
                        {
                            ...
                            Models.Complaint Complaint = context.Complaints.Find(this.expense_id);

                            if (Complaint != null & XtraMessageBox.Show(areYouSure, confirmation, MessageBoxButtons.YesNo) == DialogResult.Yes)
                            {
        ...
        {
            if (Function.Permission.HasPermission("Add Complaints"))
            {
                AddEditComplaints addEditComplaints = new AddEditComplaints();
