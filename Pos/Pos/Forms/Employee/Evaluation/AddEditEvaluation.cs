...
{
    public partial class AddEditEvaluation : DevExpress.XtraEditors.XtraForm
    {
        public Evaluations evaluations = null;
        ...
        public void setEvaluationsObject(Evaluations evaluations)
        {
        ...
        public void edit()
        {
            ...
            if (this.evaluations != null)
            {
                ...
                {
                    if (this.evaluations.evaluation_id != 0)
                        this.currentItemId = this.evaluations.evaluation_id;

                    this.evaluations.evaluation_id = 0;

                    Models.Evaluation skill = context.Evaluations.Find(this.currentItemId);
            ...
            if (dxValidationProvider.Validate())
            {
                using (var context = new AppDbContext())
                {
                    ...
                    else
                    {
                        ...
                        evaluation = context.Evaluations.Find(this.currentItemId);
                        ...
                        if (evaluation != null)
                        {
                            ...
                            context.Entry(evaluation).State = EntityState.Modified;
                        }
                        else
                        {
                            Function.Sound.Wrong();
                    ...
                    if (this.evaluations != null)
                        this.evaluations.loadEvaluations();
                }
            }
            else
            {
        ...
        private void AddEditEvaluation_Load(object sender, EventArgs e)
        {
            if (this.type != "Add")
            {
                this.edit();
        ...
        private Models.Evaluation GetCurrentData()
        {
            using (var context = new AppDbContext())
            {
                if (currentItemId != 0)
                    return context.Evaluations.Find(currentItemId);
                else
                    return null;
            }
        }
        ...
        private void DisplayCurrentItem()
        {
            using (var context = new AppDbContext())
            {
                ...
                int minId = context.Evaluations.Min(b => b.Id);
                int maxId = context.Evaluations.Max(b => b.Id);
                ...
                Models.Evaluation currentItem = GetCurrentData();
        ...
        private void MoveToFirst()
        {
            ...
            try
            {
                using (var context = new AppDbContext())
                {
                    int? minId = context.Evaluations.Min(b => (int?)b.Id);
                    if (minId.HasValue)
                    {
        ...
        private void MoveToLast()
        {
            ...
            try
            {
                using (var context = new AppDbContext())
                {
                    int? maxId = context.Evaluations.Max(b => (int?)b.Id);
                    if (maxId.HasValue)
                    {
        ...
        private void MoveToNext()
        {
            using (var context = new AppDbContext())
            {
                ...
                var nextItem = context.Evaluations.Where(b => b.Id > currentItemId).OrderBy(b => b.Id).FirstOrDefault();
                if (nextItem != null)
                {
        ...
        private void MoveToPrevious()
        {
            using (var context = new AppDbContext())
            {
                if (currentItemId != 0)
                {
                    var prevItem = context.Evaluations.Where(b => b.Id < currentItemId).OrderByDescending(b => b.Id).FirstOrDefault();
                    if (prevItem != null)
                    {
        ...
        private void InitializeSearchLookUpEdit()
        {
            ...
            using (var context = new AppDbContext())
            {
                ...
                searchLookUpEdit.Properties.DataSource = context.Evaluations.ToList();
