...
{
    public partial class AddEditSkill : DevExpress.XtraEditors.XtraForm
    {
        ...
        public void setSkillsObject(Skills skills)
        {
            this.skills = skills;
        }
        ...
        public void edit()
        {
            ...
            if (this.skills != null)
            {
                ...
                {
                    if (this.skills.skill_id != 0)
                        this.currentItemId = this.skills.skill_id;

                    this.skills.skill_id = 0;

                    Models.Skill skill = context.Skills.Find(this.currentItemId);
            ...
            if (dxValidationProvider.Validate())
            {
                using (var context = new AppDbContext())
                {
                    ...
                    else
                    {
                        ...
                        skill = context.Skills.Find(this.currentItemId);
        ...
        private void AddEditSkill_Load(object sender, EventArgs e)
        {
            if (this.type != "Add")
            {
                this.edit();
        ...
        private Models.Skill GetCurrentData()
        {
            using (var context = new AppDbContext())
            {
                if (currentItemId != 0)
                    return context.Skills.Find(currentItemId);
                else
                    return null;
            }
        }
        ...
        private void DisplayCurrentItem()
        {
            using (var context = new AppDbContext())
            {
                ...
                int minId = context.Skills.Min(b => b.Id);
                int maxId = context.Skills.Max(b => b.Id);
                ...
                Models.Skill currentItem = GetCurrentData();
        ...
        private void MoveToFirst()
        {
            ...
            try
            {
                using (var context = new AppDbContext())
                {
                    int? minId = context.Skills.Min(b => (int?)b.Id);
                    if (minId.HasValue)
                    {
        ...
        private void MoveToLast()
        {
            ...
            try
            {
                using (var context = new AppDbContext())
                {
                    int? maxId = context.Skills.Max(b => (int?)b.Id);
                    if (maxId.HasValue)
                    {
        ...
        private void MoveToNext()
        {
            using (var context = new AppDbContext())
            {
                ...
                var nextItem = context.Skills.Where(b => b.Id > currentItemId).OrderBy(b => b.Id).FirstOrDefault();
                if (nextItem != null)
                {
        ...
        private void MoveToPrevious()
        {
            using (var context = new AppDbContext())
            {
                if (currentItemId != 0)
                {
                    var prevItem = context.Skills.Where(b => b.Id < currentItemId).OrderByDescending(b => b.Id).FirstOrDefault();
                    if (prevItem != null)
                    {
        ...
        private void InitializeSearchLookUpEdit()
        {
            ...
            using (var context = new AppDbContext())
            {
                ...
                searchLookUpEdit.Properties.DataSource = context.Skills.ToList();
